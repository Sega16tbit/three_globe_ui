...
import {
	getCenter,
	offsetPoints,
	transformToEllipsoid,
} from "./FlatVertexBufferUtils.js";
...
function getInnerPoints(polygon, resolution, mode = "grid") {
	...
	const startX =
		Math.sign(_min.x) * Math.ceil(Math.abs(_min.x / resolution)) * resolution;
	const startY =
		Math.sign(_min.y) * Math.ceil(Math.abs(_min.y / resolution)) * resolution;
	...
	for (let y = startY, ly = _max.y; y < ly; y += resolution) {
		const xScalar =
			mode === "grid" ? 1 : Math.sin(Math.PI / 2 + MathUtils.DEG2RAD * y);
		const xStride = resolution / xScalar;
		const startX =
			Math.sign(_min.x) * Math.ceil(Math.abs(_min.x / xStride)) * xStride;
...
export function constructPolygonMeshObject(polygons, options = {}) {
	...
	let cleanedPolygons = polygons.map((polygon) =>
		polygon.map((loop) => loop.map((coord) => coord.slice()))
	);
	...
	const triangulations = cleanedPolygons.map((polygon) => {
		...
		if (resolution !== null) {
			...
			innerPoints = useEarcut
				? []
				: getInnerPoints(polygon, resolution, resampleMode);
